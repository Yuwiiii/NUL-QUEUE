...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sqlInsertAccountingLogs = "INSERT INTO itso_logs (queue_number, timestamp, student_id, endorsed_from, transaction, remarks, endorsed_to) 
    ...
    if ($conn->query($sqlInsertAccountingLogs) === TRUE) {
        ...
        $sqlInsertQueueLogs = "INSERT INTO queue_logs (queue_number, student_id, office, remarks, endorsed) 
                               VALUES ('$queueNumber', '$studentID', 'Itso', '$remarks', 'Completed')";
        ...
        if ($conn->query($sqlUpdateStatus) !== TRUE) {
            ...
            if ($conn->query($sqlUpdateDisplay) !== TRUE) {
                ...
                $sqlDeleteAccounting = "DELETE FROM itso WHERE queue_number = '$queueNumber'";
