...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sqlInsertAccountingLogs = "INSERT INTO clinic_logs (queue_number, timestamp, student_id, endorsed_from, transaction, remarks, endorsed_to) 
    ...
    if ($conn->query($sqlInsertAccountingLogs) === TRUE) {
        ...
        if ($conn->query($sqlUpdateStatus) !== TRUE) {
            ...
            if ($conn->query($sqlUpdateDisplay) !== TRUE) {
                ...
                $sqlDeleteAccounting = "DELETE FROM clinic WHERE queue_number = '$queueNumber'";
